...
- [ ] 9.0 Integrate MergeProgress with real API status polling
  ...
  - [ ] 9.2 Update `startMerge()` to use actual merge initiation (pass mergeId from parent)
  - [ ] 9.3 Replace `simulateMergeProgress()` with real polling using `mergeApiService.getMergeStatus(sourceAppId, { mergeId })`
  - [ ] 9.4 Set up polling interval at 5 seconds (per feedback, not 2 seconds)
  - [ ] 9.5 Parse status response per `docs/05-api-tech-spec.md` structure
  - [ ] 9.6 Map progress percentage from API response
  - [ ] 9.7 Fetch logs separately via `mergeApiService.fetchMergeLogs(sourceAppId, { mergeId })`
  - [ ] 9.8 Parse logs to determine current phase based on log types (per feedback: use audit log types to determine phase)
  - [ ] 9.9 Map log entries to activity messages with correct status icons
  - [ ] 9.10 Update `currentPhaseLabel` based on latest log type
  - [ ] 9.11 Remove simulated phases array (lines 316-324)
  - [ ] 9.12 Stop polling when status is "completed" or "error"
  - [ ] 9.13 Pass mergeId as request body parameter, not query param (per feedback line 307)
  - [ ] 9.14 Clean up polling interval on component unmount

- [ ] 10.0 Integrate MergeComplete with real API results
  - [ ] 10.1 Import `MergeApiService` in MergeComplete.vue
  - [ ] 10.2 Accept mergeId as prop from parent component
  - [ ] 10.3 Replace mock data in `loadMergeResults()` with final status call: `mergeApiService.getMergeStatus(sourceAppId, { mergeId })`
  - [ ] 10.4 Parse summary counts from status response per `docs/05-api-tech-spec.md` structure
  - [ ] 10.5 Map API field names to UI display names for counts
  - [ ] 10.6 Extract issues/warnings from final status response
  - [ ] 10.7 Extract plan limit warnings from `limitWarnings` field in status response
  - [ ] 10.8 Fetch merge history: call `mergeApiService.fetchMergeLogs(sourceAppId, { types: ['app.merge.initiated'] })`
  - [ ] 10.9 Parse log entries to build previousMerges array with fields: id, completedAt, sourceAppName, targetAppName, itemsCount, status
  - [ ] 10.10 Extract merge IDs from log data to show recent merge operations
  - [ ] 10.11 Format dates using `formatDate()` helper
  - [ ] 10.12 Remove mock data (lines 338-367)
  - [ ] 10.13 Test completion view with successful and failed merges
