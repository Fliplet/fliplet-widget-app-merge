...
```javascript
// Function to register all design system components on a Vue app instance
window.registerMergeComponents = function(app) {
  if (!app || !app.component) {
    console.error('Invalid Vue app instance passed to registerMergeComponents');
    return;
  }
      ...
      <div :class="['merge-lock-timer', { 'warning': isWarning, 'expired': isExpired }]">
        ...
        <template v-else>
          ...
          <span class="expired-text">
            <strong>Lock Expired!</strong> Please return to the dashboard.
          </span>
        </template>
      </div>
    `
  ...
  console.log('Design System components registered on app instance');
  return app;
};
```

## Design System Showcase - Demo Data Requirements

When creating the Design System showcase screen, use proper timestamps to ensure all component states are visible and testable.

### Lock Timer Demo Data

The Lock Timer component requires future timestamps to properly display its various states. Use these patterns:

```javascript
// Normal state: 5 minutes from now (shows gray background with countdown)
const normalStateExpiry = Date.now() + (5 * 60 * 1000);

// Warning state: 90 seconds from now (shows orange background with "Extend" button)
const warningStateExpiry = Date.now() + (90 * 1000);

// Expired state: 1 minute ago (shows red background with expired message)
const expiredStateExpiry = Date.now() - (60 * 1000);
```

### Implementation in Showcase

In the Design System showcase screen JavaScript:

```javascript
// In screen's data() function
data() {
  return {
    // Timer demo timestamps
    normalLockExpiry: Date.now() + (5 * 60 * 1000),
    warningLockExpiry: Date.now() + (90 * 1000),
    expiredLockExpiry: Date.now() - (60 * 1000)
  };
}
```
...
```html
<h3>Normal state (>2 minutes remaining):</h3>
<merge-lock-timer :locked-until="normalLockExpiry" />

<h3>Warning state (<2 minutes remaining):</h3>
<merge-lock-timer :locked-until="warningLockExpiry" />

<h3>Expired state:</h3>
<merge-lock-timer :locked-until="expiredLockExpiry" />
```

### Why This Matters

- **Expired timestamps show no countdown**: If demo timestamps are in the past, all states appear expired
- **Future timestamps enable testing**: Developers can observe real countdown behavior
- **Consistent across refreshes**: Timestamps should be calculated on page load, not hardcoded
...
## Using the Components in Screens

To use the design system components in a screen, register them on your Vue app instance:

```javascript
$(document).ready(function() {
  (function initApp() {
    if (typeof Vue === 'undefined' || !Vue || typeof window.registerMergeComponents !== 'function') {
    ...
    window.registerMergeComponents(app);

    // Mount the app
    app.mount('#your-app-container');
  })();
});
```

**Key points:**
- Always call `window.registerMergeComponents(app)` before mounting
- This makes all 9 design system components available in your templates
- Components use kebab-case in templates: `<merge-button>`, `<merge-card>`, etc.
