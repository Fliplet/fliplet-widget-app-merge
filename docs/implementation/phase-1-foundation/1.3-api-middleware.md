...
**CRITICAL**: Use `Fliplet.API.request()` instead of raw `fetch()`:

```javascript
// ✅ CORRECT - Fliplet's API method
...
All middleware functions return promises that reject with structured error objects:
